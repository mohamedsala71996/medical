...
@section('content')
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">تعديل مدير </h4>
                <form class="forms-sample" method="post" action="{{route('admins.update',$admin->id)}}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-group" @error('name') has-danger @enderror>
                        <label for="name">الاسم</label>
                        <input data-validation="required" name="name" value="{{$admin->name}}" type="text" class="form-control" id="name" placeholder="الاسم">
                        @error('name') <small class="form-control-feedback"> </small> @enderror
                    </div>
                    <div class="form-group" @error('email') has-danger @enderror>
                        <label for="email">البريد الالكترونى</label>
                        <input data-validation="required,email" type="email" class="form-control" value="{{$admin->email}}" name="email" id="email" placeholder="البريد الالكترونى">
                        @error('email') <small class="form-control-feedback"> </small> @enderror

                    </div>
                    <div class="form-group" @error('phone') has-danger @enderror>
                        <label for="phone">رقم الجوال</label>
                        <input data-validation="required,number" type="text" class="form-control" value="{{$admin->phone}}" name="phone" id="phone" placeholder="رقم الجوال">
