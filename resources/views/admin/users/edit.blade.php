            ...
            <div class="card-body">
                ...
                <form class="forms-sample" method="post" action="{{route('users.update',$user->id)}}" enctype="multipart/form-data">
                    ...
                    <div class="form-group" @error('name') has-danger @enderror>
                        ...
                        <input data-validation="required" name="name" value="{{$user->name}}" type="text" class="form-control" id="name" placeholder="الاسم">
                        @error('name') <small class="form-control-feedback"> </small> @enderror
                    </div>
                    <div class="form-group" @error('email') has-danger @enderror>
                        <label for="email">البريد الالكترونى</label>
                        <input data-validation="required,email" type="email" class="form-control" value="{{$user->email}}" name="email" id="email" placeholder="البريد الالكترونى">
                        ...
                        <select data-validation="required" id="phone_code" name="phone_code" class="select2 m-b-10 form-control">
                            ...
                            @foreach ($codes as $code)
                                <option value="{{ $code->phone_code }}"
                                        {{ $user->phone_code == $code->phone_code ? 'selected' : "" }}>
                                    ( {{ $code->phone_code }} +)
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group" @error('phone') has-danger @enderror>
                        ...
                        <input data-validation="required,number" type="text" class="form-control" value="{{$user->phone}}" name="phone" id="phone" placeholder="رقم الجوال">
                    ...
                    <div class="form-group">
                        <label>الصورة</label>
                        <input  type="file" name="logo" id="input-file-now-custom-1" class="dropify" data-default-file="{{get_file($user->logo)}}" />
                   ...
                   {{-- <input type="hidden" value="1" name="country_id">
                    <div class="form-group">
                        ...
                        <select  id="city_id" name="city_id" class="select2 m-b-10 form-control">
                            @foreach($cities as $city)
                                <option {{$user->city_id==$city->id_city?'selected':''}} value="{{$city->id_city}}" >{{$city->ar_city_title}}</option>
                            @endforeach
                        </select>
