...
@endphp
                ...
                <div class="new-postbox">
                    <div class="newpst-input">
                        <form method="post" action="{{ route('posts.store') }}">
                            @csrf
                            <textarea name="body" rows="3" placeholder="{{ __('Write something') }}"></textarea>
    ...
    <div class="central-meta item">
        <div class="user-post">
            <h2 class="text-center">{{ __('News') }}</h2>
        </div>
    </div>
    @if ($posts->count() > 0)
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                    data-bs-target="#editPostModal{{ $post->id }}">{{ __('Edit') }}</a></li>
                                            <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                    data-bs-target="#deletePostModal{{ $post->id }}">{{ __('Delete') }}</a></li>
                                        </ul>
                                    </div>
                                    ...
                                    <li>
                                        ...
                                        <div class="we-comment">
                                            <div class="coment-head">
                                                ...
                                                <a class="we-reply" href="#" title="{{ __('Reply') }}"><i class="fa fa-reply"></i></a>
                                            </div>
                                            <p>{{ $comment->comment }}</p>
                                            @if (Auth::check() && auth()->user()->id == $comment->user_id)
                                                <a class="btn btn-danger px-sm-1 rounded-3" href="#" data-bs-toggle="modal" data-bs-target="#deleteCommentModal{{ $comment->id }}">{{ __('Delete') }}</a>
                                ...
                                <li class="post-comment">
                                    ...
                                    <div class="post-comt-box">
                                        <form action="{{ route('comments.store') }}" method="post">
                                            @csrf
                                            <textarea name="comment" rows="3" placeholder="{{ __('Post your comment') }}"></textarea>
                                            <input type="hidden" name="post_id" value="{{ $post->id }}">
                                            <br>
                                            <div class="friend-name" style="margin-top: 5px ">
                                                <button class="left btn btn-outline-dark btn-sm" type="submit">{{ __('Create') }}</button>
                                            </div>
                                        </form>
                                    </div>
                                </li>
